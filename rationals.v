...
Section Integers.
    ...
    Notation "1" := (succ 0) : Int_scope.
    Notation "2" := (succ 1) : Int_scope.
    Notation "-1" := (pred 0) : Int_scope.
    ...
    Proof.
        ...
        - apply f_equal with (f := fun t => pred t). apply IHn.
    Qed.
    ...
    Lemma add_succ_l : forall (n m : Int), succ (n + m) = (succ n) + m.
    Proof.
        intros. simpl. reflexivity.
    Qed.
    Hint Resolve add_succ_l : hdb.

    Lemma add_succ_r : forall (n m : Int), succ (n + m) = n + (succ m).
    Proof.
        intros. induction n; simpl.
        - reflexivity.
        - rewrite <- IHn. reflexivity.
        - rewrite <- IHn, succ_pred, pred_succ. reflexivity.
    Qed.
    Hint Resolve add_succ_r : hdb.
  
    Lemma add_pred_l : forall (n m : Int), pred (n + m) = (pred n) + m.
    Proof.
        intros. simpl. reflexivity.
    Qed.
    Hint Resolve add_pred_l : hdb.

    Lemma add_pred_r : forall (n m : Int), pred (n + m) = n + (pred m).
    Proof.
        intros. induction n; simpl.
        - reflexivity.
        - rewrite <- IHn, succ_pred, pred_succ. reflexivity.
        - rewrite <- IHn. reflexivity.
    Qed.
    Hint Resolve add_pred_r : hdb.
    
    Theorem add_assoc : 
        forall (i j k : Int), i + (j + k) = (i + j) + k.
    Proof.
        intros. induction i.
        all: simpl; try rewrite <- IHi; reflexivity.
    Qed.
    ...
    Proof.
        ...
        - rewrite IHn. apply add_pred_r.
    Qed.
    Hint Resolve add_comm : hdb.
    
    Lemma add_1_l : forall (n : Int), succ n = 1 + n.
    Proof.
        intro. simpl. reflexivity.
    Qed.
    Hint Resolve add_1_l : hdb.

    Lemma add_1_r : forall (n : Int), succ n = n + 1.
    Proof.
        intro. rewrite add_comm. simpl. reflexivity.
    Qed.
    Hint Resolve add_1_r : hdb.
    ...
    Theorem add_normalise_l : forall (i j k : Int), i = j <-> k + i = k + j.
    Proof.
        split.
        intro. apply f_equal with (f := fun t => k + t) in H. apply H.
        intro. induction k; simpl in H.
        - apply H.
        - apply f_equal with (f := fun t => pred t) in H.
          repeat rewrite pred_succ in H. apply (IHk H).
        - apply f_equal with (f := fun t => succ t) in H.
          repeat rewrite succ_pred in H. apply (IHk H).
    Qed.
    Hint Resolve add_normalise_l : hdb.

    Theorem add_normalise_r : forall (i j k : Int), i = j <-> i + k = j + k.
    Proof.
        split.
        ...
        - apply f_equal with (f := fun t => pred t) in H.
          repeat rewrite <- add_succ_r, pred_succ in H.
          simpl in H. apply (IHk H).
        - apply f_equal with (f := fun t => succ t) in H.
          repeat rewrite <- add_pred_r, succ_pred in H.
          simpl in H. apply (IHk H).
    Qed.
    Hint Resolve add_normalise_r : hdb.
  
    Lemma add_permute : forall (i j k : Int), i + j + k = j + k + i.
    Proof.
        intros.
        rewrite <- add_comm, add_assoc, 
            <- (add_assoc j k i), (add_comm j).
        reflexivity.
    Qed.
    ...
    Proof.
        intro. induction n; simpl; try rewrite IHn; reflexivity.
    Qed.
    ...
    Lemma sub_1_r : forall (n : Int), pred n = n - 1.
    Proof.
        intro. rewrite add_comm. simpl. reflexivity.
    Qed.
    Hint Resolve sub_1_r : hdb.

    Lemma sub_1_l : forall (n : Int), pred n = - (1) + n.
    Proof.
        intro. rewrite add_comm. apply sub_1_r.
    Qed.
    Hint Resolve sub_1_r : hdb.
    
    Theorem add_inv_r : forall (n : Int), n - n = zero.
    Proof.
        intros. induction n; simpl.
        - reflexivity.
        - rewrite <- add_pred_r, IHn. apply succ_pred.
        - rewrite <- add_succ_r, IHn. apply pred_succ.
    Qed.
    Hint Resolve add_inv_r : hdb.

    Theorem add_inv_l : forall (n : Int), - n + n = zero.
    Proof.
        intros. rewrite add_comm. apply add_inv_r.
    Qed.
    Hint Resolve add_inv_l : hdb.
    
    Lemma opp_sum : forall (n m : Int), - (n + m) = (- n) + (- m).
    Proof.
        ...
        - rewrite <- opp_succ.
          induction n; simpl.
          + reflexivity.
          + rewrite <- (opp_succ m). rewrite IHn. reflexivity.
        ...
        - rewrite <- add_succ_r. rewrite <- add_pred_r. rewrite opp_pred.
          apply -> equiv_succ. apply IHm. 
    Qed.
    Hint Resolve opp_sum : hdb.
    ...
    Proof.
        ...
        rewrite <- add_assoc, add_inv_r, add_zero_r in H. assumption.
    Qed.
    
    Lemma add_inv_implies_eq : forall n m, n = m <-> n - m = 0.
    Proof.
        split.
        intro. apply f_equal with (f := fun t => t - m) in H.
        rewrite add_inv_r in H. assumption.
        intro. apply swap_sides_eqn_sum in H. simpl in H.
        rewrite opp_opp in H. assumption.
    Qed.
    ...
    Proof.
        ...
        rewrite add_1_r. rewrite <- add_permute. rewrite <- add_1_r.
    ...
    Proof.
        ...
        repeat rewrite <- add_assoc in H. rewrite add_inv_r in H.
        repeat rewrite add_zero_r in H. assumption.
    Qed.
    
    Lemma lt_not_id : forall n, lt n n -> False.
    Proof.
        intros. unfold lt in H. rewrite add_1_r in H. 
    ...
    Proof.
        ...
        replace (succ k) with (k + 1) in G. rewrite add_1_r in G.
    ...
    Proof.
        ...
        apply lte_s in G. rewrite add_1_r in G.
    ...
    Proof.
        ...
        apply lte_s in Ilt.
        rewrite add_1_r in Ilt. replace (succ j) with (j + 1) in Ilt.
    ...
    Proof.
        ...
        left. reflexivity.
        right. right.
        rewrite (add_1_r (succ n)). rewrite (add_1_r m).
        ...
        apply lte_s in Cn.
        rewrite (add_1_r zero) in Cn. rewrite add_1_r in Cn.
    ...
    Proof.
        ...
        inversion H. apply pos_one. rewrite add_1_r. apply pos_s.
        apply IHn. assumption.
        
        induction H. apply pos_one. rewrite add_1_r. apply pos_s.
        assumption.
        
        intro. induction H. apply (lte_id 1).
        unfold positive. rewrite <- add_1_r. apply lte_s.
        assumption.
    Qed.
    ...
    Proof.
        ...
        rewrite <- sub_1_r. unfold negative, lt. rewrite succ_pred.
    ...
    Proof.
        ...
        induction CP. rewrite <- add_1_r. apply lte_s. assumption.
        rewrite add_assoc. rewrite <- add_1_r. apply lte_s.
        assumption.
    Qed.
    ...
    Proof.
        ...
        all: rewrite <- add_1_r; apply lte_s; assumption.
    Qed.
    ...
    Proof.
        ...
        simpl. rewrite <- add_1_r, IHlte. reflexivity.
    Qed.
    ...
    Proof.
        ...
        all: simpl; rewrite <- add_pred_r || rewrite <- add_succ_r; simpl;
    ...
    Proof.
        ...
        rewrite opp_succ. rewrite <- add_pred_r. simpl. rewrite add_assoc.
        apply -> lte_invar_lr_add. assumption.
        
        rewrite opp_pred. rewrite <- add_succ_r. simpl. rewrite add_assoc.
        apply -> lte_invar_lr_add. assumption.
        
        simpl. rewrite <- add_pred_r. simpl. 
        rewrite add_assoc, <- add_1_r, <- add_permute, <- add_1_r.
        apply lte_s, lte_s.
        replace (abs (j - k) + abs (i - j)) with (abs (i - j) + abs (j - k)).
        assumption. rewrite add_comm. reflexivity.
        
        simpl. rewrite <- add_succ_r. simpl. 
        rewrite add_assoc, <- add_1_r, <- add_permute, <- add_1_r.
        apply lte_s, lte_s.
        replace (abs (j - k) + abs (i - j)) with (abs (i - j) + abs (j - k)).
        assumption. rewrite add_comm. reflexivity.
    ...
    Lemma multiply_0_l : forall (n : Int), zero * n = zero.
    Proof.
        intro. trivial.
    Qed.

    Lemma multiply_0_r : forall (n : Int), n * zero = zero.
    Proof.
        intro. induction n.
        simpl. reflexivity.
    ...
    Lemma multiply_1_l : forall (n : Int), 1 * n = n.
    Proof.
        intro. trivial.
    Qed.

    Lemma multiply_1_r : forall (n : Int), n * 1 = n.
    Proof.
        intro. simpl. induction n. trivial.
        all: simpl; rewrite IHn;
        rewrite <- add_succ_r || rewrite <- add_pred_r;
        rewrite add_zero_r; reflexivity.
    Qed.
    
    Lemma multiply_succ_l : forall (n m : Int), (succ n) * m = n * m + m.
    Proof.
        intros. trivial.
    Qed.

    Lemma multiply_succ_r : forall (n m : Int), n * (succ m) = n * m + n.
    Proof.
        intros. induction n.
        simpl. reflexivity.
        simpl. rewrite IHn. rewrite add_permute, <- add_succ_r.
        symmetry. rewrite add_permute, <- add_succ_r.
        apply add_normalise_r. apply -> equiv_succ. apply add_comm.
        
        simpl. rewrite IHn. rewrite add_permute, <- add_pred_r.
        symmetry. rewrite add_permute, <- add_pred_r.
        apply add_normalise_r. apply -> equiv_pred. apply add_comm.
    Qed.
    
    Lemma multiply_pred_l : forall (n m : Int), (pred n) * m = n * m - m.
    Proof.
        intros. trivial.
    Qed.

    Lemma multiply_pred_r : forall (n m : Int), n * (pred m) = n * m - n.
    Proof.
        intros. induction n.
        simpl. reflexivity.
        simpl. rewrite IHn. rewrite add_permute, <- add_pred_r.
    ...
    Lemma multiply_sum_l : forall (i j k : Int), (i + j) * k = i * k + j * k.
    Proof.
        intros. induction k. repeat rewrite multiply_0_r. trivial.
        repeat rewrite multiply_succ_r. rewrite IHk.
        rewrite <- add_assoc, <- (add_assoc (i*k) i _).
        apply -> add_normalise_l. rewrite add_comm, <- add_assoc.
        apply -> add_normalise_l. apply add_comm.
    Qed.

    Lemma multiply_sum_r : forall (i j k : Int), i * (j + k) = i*j + i*k.
    Proof.
        intros. induction i.
        simpl. reflexivity.
        simpl. rewrite IHi. repeat rewrite add_assoc.
        apply add_normalise_r. rewrite <- add_permute.
        apply add_normalise_r. apply add_comm.
        simpl. rewrite opp_sum.
        replace (i * j - j + (i * k - k)) with (i * j + i * k + (- j - k)).
        apply add_normalise_r. apply IHi.
        rewrite add_permute. symmetry. rewrite add_permute.
        repeat rewrite add_assoc.
        repeat (apply add_normalise_r || apply add_comm).
    Qed.
    
    Lemma multiply_opp_r : forall (n m : Int), n * (- m) = - (n * m).
    Proof.
        intros. induction m.
        simpl. rewrite multiply_0_r. trivial.
        all: simpl; rewrite multiply_succ_r, multiply_pred_r, opp_sum, IHm; 
    ...
    Proof.
        intros. induction m.
        simpl. repeat rewrite multiply_0_r. trivial.
        rewrite multiply_succ_r, multiply_succ_r, opp_sum, IHm. reflexivity.
        rewrite multiply_pred_r, multiply_pred_r, opp_sum, IHm. reflexivity.
    Qed.
    
    Lemma multiply_sum_r_sub : forall i j k, i * (j - k) = i * j - i * k.
    Proof.
        intros. rewrite multiply_sum_r, multiply_opp_r. reflexivity.
    Qed.
    
    Theorem multiply_comm : forall (n m : Int), n * m = m * n.
    Proof.
        intros. induction m. 
        rewrite multiply_0_r. simpl. reflexivity.
        rewrite multiply_succ_r. simpl. rewrite IHm. reflexivity.
        simpl. replace (pred m) with (pred (m + 0)).
        rewrite add_pred_r. rewrite multiply_sum_r. simpl.
        rewrite <- opp_one, multiply_opp_r, multiply_1_r, IHm. reflexivity.
        rewrite add_zero_r. reflexivity.
    Qed. 
    ...
    Proof.
        ...
        rewrite multiply_sum_r. rewrite multiply_comm.
        rewrite add_comm. symmetry. rewrite add_comm.
        rewrite IHi. apply add_normalise_r, multiply_comm.
        
        simpl. symmetry. rewrite multiply_comm, multiply_sum_r, multiply_opp_r.
        replace (k * j) with (j * k).
        apply add_normalise_r. rewrite multiply_comm.
    ...
    Proof.
        ...
        apply add_comm.
        apply add_assoc.
        rewrite multiply_comm, multiply_1_r. reflexivity.
        apply multiply_comm.
        apply multiply_assoc.
        rewrite multiply_comm, multiply_sum_r, multiply_comm, add_comm, multiply_comm, add_comm. reflexivity.
        reflexivity.
        apply add_inv_r.
    Qed.
    ...
    Proof.
        ...
        rewrite add_1_r. 
        unfold positive, lt. rewrite add_1_r, add_assoc.
        ...
        rewrite add_comm, add_inv_r in H. simpl in H. assumption.
        
        unfold positive, negative, lt in H, IHk.
        rewrite sub_1_r.
        unfold positive, lt.
        rewrite opp_pred, add_1_r, add_1_r in H.
        ...
        apply add_normalise_r. rewrite add_comm, add_inv_r. reflexivity.
        replace (-1 + 1) with 0. reflexivity.
        rewrite add_comm, add_inv_r. reflexivity.
    Qed.*)
    ...
    Proof.
        ...
        rewrite sub_1_r in H.
        apply lte_invar_lr_add with (n := 1) in H.
        rewrite opp_one, <- (add_assoc n -1 1), add_inv_r, add_zero_r in H.
    ...
    Proof.
        ...
        replace (n - 1) with (pred n) in H by (rewrite sub_1_r; reflexivity).
        assumption.
    Qed.
    ...
    Proof.
        ...
        rewrite add_inv_r in Hcon. apply zero_signless in Hcon.
        contradiction.
    Qed.
    ...
    Proof.
        ...
        replace (succ n) with (n + 1) by (rewrite <- add_1_r; reflexivity).
        replace (succ (n + m)) with (n + m + 1) by (rewrite <- add_1_r; reflexivity).
        unfold lt. replace (succ (n + 1)) with ((succ n) + 1) by trivial.
        apply lte_invar_lr_add with (n := 1). assumption.
        
        unfold lt. rewrite succ_pred. simpl.
        apply lte_invar_lr_sub with (k := 1).
        replace (n + 1) with (succ n) by (rewrite add_1_r; reflexivity).
        simpl. rewrite <- (add_succ_r (n + m)), pred_succ, add_zero_r.
        assumption.
    Qed.
    ...
    Proof.
        ...
        apply lte_invar_lr_sub with (k := 1).
        replace (n + m + 1) with (succ (n + m)) by (rewrite add_1_r; reflexivity).
        replace (pred n + 1) with (n) by
        (rewrite <- add_1_r, succ_pred; reflexivity).
        assumption.
    Qed.
    ...
    Proof.
        ...
        induction G. rewrite <- add_1_r. trivial.
        inversion H. simpl. symmetry. rewrite add_1_r. reflexivity.
        
        rewrite H1. rewrite <- add_succ_r. simpl.
    ...
    Proof.
        ...
        destruct_sign m.
        rewrite Cz in H. rewrite multiply_0_r in H.
        apply zero_signless in H. contradiction.
    ...
    Proof.
        ...
        apply lte_s in H.
        rewrite add_1_r, (add_1_r n) in H.
        apply lte_invar_lr_sub in H.
        assumption.
    Qed.
    ...
    Proof.
        ...
        simpl. repeat rewrite multiply_0_r. simpl. reflexivity.
    ...
    Proof.
        intros. induction H1.
        rewrite multiply_1_r. assumption.
        rewrite multiply_succ_r. apply lte_invar_r_add.
        all: assumption.
    Qed.
    ...
    Proof.
        ...
        rewrite G, multiply_0_r. reflexivity.
    Qed.
    ...
    Proof.
        ...
        rewrite multiply_0_r in H. discriminate.
        ...
        rewrite Cm1 in H. rewrite multiply_1_r in H.
        left. split; assumption.
        
        apply lte_min_or_more in m_min.
        destruct m_min as [Cmn1 | m_min].
        rewrite <- Cmn1 in H. rewrite multiply_opp_r, multiply_1_r in H.
        ...
        rewrite Hcon, multiply_0_r in H. discriminate.
    Qed.
    ...
    Proof.
        intros i j k Hkz H.
        generalize (eq_implies_add_inv_r _ _ H).
        intro. rewrite <- multiply_sum_r_sub in H0.
        destruct (product_zero_implies_zero _ _ H0).
        contradiction.
        apply add_inv_r_implies_eq in H1. assumption.
    Qed.
    ...
    Proof.
        ...
        repeat rewrite multiply_1_r in H. assumption.
    Qed.
    ...
    Proof.
        ...
        repeat rewrite multiply_1_r. rewrite add_zero_r.
